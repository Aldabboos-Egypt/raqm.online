        ...
        <div class="modal-body row">
            ...
            @foreach (config('translatable.locales') as $locale)
                <div class="col-md-6 col-sm-6 mb-2 pt-2">
                    ...
                    {!! Form::text("$locale" . '[title]', old($locale . '.title'), [
                        'class' => 'form-control',
            ...
            @foreach (config('translatable.locales') as $locale)
                <div class="col-md-6 col-sm-6 mb-2 pt-2">
                    ...
                    {!! Form::text("$locale" . '[description]', old($locale . '.description'), [
                        'class' => 'form-control',
                        'placeholder' => __('lang.' . $locale . '.description'),
                    ]) !!}
                    <div class="invalid-feedback">{{ __('lang.please_enter_valid_value') }}.</div>
                </div>
            @endforeach
            ...
            <div class="col-md-6 col-sm-12 mb-2 pt-2">
                ...
                <select name="page" id="page" class="form-select">
                    ...
                    @foreach ($pages as $key => $page)
                        <option value="{{ $page }}" {{ old('page') == $page ? 'selected' : '' }}>
                            {{ $page }}</option>
                    @endforeach
                </select>
            </div>
