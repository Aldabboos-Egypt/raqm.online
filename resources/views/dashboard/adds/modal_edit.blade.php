{!! Form::open([
    'route' => ['dashboard.adds.update', $add->id],
    'method' => 'post',
    'files' => true,
    'class' => '',
]) !!}
        ...
        <div class="modal-body row">
            ...
            @foreach (config('translatable.locales') as $locale)
                <div class="col-md-6 col-sm-6 mb-2 pt-2">
                    ...
                    {!! Form::text("$locale" . '[title]', old($locale . '.title', $add->translate($locale)->title), [
                        'class' => 'form-control',
                        'placeholder' => __('lang.' . $locale . '.title'),
                    ]) !!}
                    <div class="invalid-feedback">{{ __('lang.please_enter_valid_value') }}.</div>
                </div>
            @endforeach
            ...
            @foreach (config('translatable.locales') as $locale)
                <div class="col-md-6 col-sm-6 mb-2 pt-2">
                    ...
                    {!! Form::text(
                        "$locale" . '[description]',
                        old($locale . '.description', $add->translate($locale)->description),
                        ['class' => 'form-control', 'placeholder' => __('lang.' . $locale . '.description')],
                    ) !!}
                    <div class="invalid-feedback">{{ __('lang.please_enter_valid_value') }}.</div>
                </div>
            @endforeach
            ...
            <div class="col-md-6 col-sm-12 mb-2 pt-2">
                ...
                <select name="page" id="page" class="form-select">
                    ...
                    @foreach ($pages as $key => $page)
                        <option value="{{ $page }}" {{ $selectedPage == $page ? 'selected' : '' }}>
                            {{ $page }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="col-md-6 col-sm-12 mb-2 pt-2">
                ...
                {!! Form::date('date_from', old('date_from', $add->date_from), [
                    'class' => 'form-control',
                    'placeholder' => __('lang.date_from'),
                ]) !!}
            </div>

            <div class="col-md-6 col-sm-12 mb-2 pt-2">
                {!! Form::label('date_to', __('lang.date_to')) !!}
                {!! Form::date('date_to', old('date_to', $add->date_to), [
                    'class' => 'form-control',
                    'placeholder' => __('lang.date_to'),
                ]) !!}
            </div>
